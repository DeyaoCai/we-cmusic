...
Drag.prototype.touchEv = function touchEv(ev) {
...
Drag.prototype.moveEv = function moveEv(ev){
  ...
  this.drection ||
    (this.drection =
      Math.abs(this.now.x - this.pre.x) > Math.abs(this.now.y - this.pre.y) ?
        "x" : "y"
    );
  // 如果是不阻止  则不做处理
  // 如果阻止其中任一个方向 
...
Drag.prototype.endEv = function endEv(){
  this.touched = false;
  this.isTouching = false;
}
Drag.prototype.getOffset = function getOffset(){
  return {
    x: this.now.x - this.ori.x,
    y: this.now.y - this.ori.y,
  }
}
Drag.prototype.getSpeed = function getSpeed(){
  return {
